...
func command(line string) {
	...
	case "/peers_leader":
		fmt.Printf("\n")
		fmt.Printf("Peer:%s Leader: %s\n", node.ID().Address, overlay.GetLeader())
		...
		_, err = conn.Write([]byte(overlay.GetLeader() + "|" + node.ID().Address))
		if err != nil {
			log.Fatal(err)
		}
		return
	...
	//publisher command
		...
		pjt, found := overlay.PubSub().GetProject(my_pjt_id)
			...
			//JL - show whole sublist
			//fmt.Printf("The Sublist: %s\n", pjt.GetSubList())
		}
                return
...
func RequestGroup() {
	...
	i := 0
		...
		//JL
		fmt.Printf("sublist[%s] address[%s] traintime:%d numleave:%d\n", tx.Key[:8], string(tx.Addr), tx.UserInfo.(UserInfo).TrainTime, tx.UserInfo.(UserInfo).NumLeave)
	    i = i + 1
        }
    ...
    //     _, err = conn.Write([]byte(peer.ID().String() + "|" + peer.Overlay().GetLeader()))
    //     if err != nil {
    //         log.Fatal(err)
    //     }
    // }
